...
dir_w2vdata = '/home/uww/fastText/w2vdata'
...
def load_w2v_gensim(duanluo, anyou):
    ...
    """
    print 'load model...'
    file_w2v_model_xingshi = dir_w2vdata + '/xingshi/w2vgensim/' + duanluo + '/W2VSz100SgNeg' + anyou + '.model.bin'
    if os.path.isfile(file_w2v_model_xingshi):
        print 'model loaded'
        return KeyedVectors.load_word2vec_format(file_w2v_model_xingshi, binary=True)

    file_w2v_model_minshi = dir_w2vdata + '/minshi/w2vgensim/' + duanluo + '/W2VSz100SgNeg' + anyou + '.model.bin'
...
def load_glove(duanluo, anyou):
    ...
    """
    print 'load model...'
    file_glove_model_xingshi = dir_w2vdata + '/xingshi/glove/' + duanluo + '/W2VSz100SgNeg' + anyou + '.txt'
    if os.path.isfile(file_glove_model_xingshi):
        print 'model loaded'
        return KeyedVectors.load_word2vec_format(file_glove_model_xingshi, binary=False)

    file_glove_model_minshi = dir_w2vdata + '/minshi/glove/' + duanluo + '/W2VSz100SgNeg' + anyou + '.txt'
...
def load_w2v_google(duanluo, anyou):
    ...
    """
    print 'load model...'
    file_w2v_model_xingshi = dir_w2vdata + '/xingshi/w2vgoogle/' + duanluo + '/W2VSz100SgNeg' + anyou + '.bin'
    if os.path.isfile(file_w2v_model_xingshi):
        print 'model loaded'
        return word2vec.load(file_w2v_model_xingshi)

    file_w2v_model_minshi = dir_w2vdata + '/minshi/w2vgoogle/' + duanluo + '/W2VSz100SgNeg' + anyou + '.bin'
    if os.path.isfile(file_w2v_model_minshi):
        print 'model loaded'
...
class WordVectorDemoHandler(tornado.web.RequestHandler):
    def get(self):
        self.write('<html><body><form action="/wordVector" method="post">'
                   ...
                   '<select name="method" id="select_model">'
                   ...
                   '<select name="duanluo" id="select_duanluo">'
                   ...
                   '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
                   '<span>案由</span>'
                   '<input id="anyou" type="text" name="anyou" class="inputbox"'
...
class WordVectorHandler(tornado.web.RequestHandler):
    def get(self):
        pass
...
if __name__ == "__main__":
    ...
    cp.read('w2vserver.conf')
    server_port = cp.get('service', 'port')
    dir_w2vdata = cp.get('data_path', 'dir')

    application = tornado.web.Application([
        (r'/analogyWords', AnalogyWordsHandler),
        (r'/wordVectorDemo', WordVectorDemoHandler),
        (r'/wordVector', WordVectorHandler)
    ])
